...
describe("Round set up", () => {
  ...
  it("deals 7 cards to each player from the top of the deck", () => {
    ...
    for (let playerIndex = 0; playerIndex < round.playerCount; playerIndex += 1) {
      round.playerHand(playerIndex).forEach((card, index) => {
        expect(card).toEqual(cards[7 * playerIndex + index])
      })
    }
  })
  ...
  it("keeps the undealt cards in the draw pile", () => {
    const undealtCards = cards.slice(dealtCardsCount)
    for (let index = 1; index < undealtCards.length; index += 1) {
      expect(round.drawPile().deal()).toEqual(undealtCards[index])
    }
  })
