...
export class GameService {
  ...
  readonly #liveRounds = new Map<string, Round>()
  ...
  async login(username: string, password: string): Promise<AuthPayload> {
    ...
    if (player === undefined || !passwordMatches(password, player)) throw new Error("Invalid username or password")
    return this.createSession(player)
  }
  ...
  async createGame(token: string | undefined, name: string, maxPlayers: number): Promise<GameView> {
    ...
    if (!Number.isInteger(maxPlayers) || maxPlayers < 2 || maxPlayers > 4) throw new Error("A game must allow between 2 and 4 players")
    const trimmedName = name.trim()
    if (trimmedName.length < 1 || trimmedName.length > 50) throw new Error("Game name must contain between 1 and 50 characters")
  ...
  async startGame(token: string | undefined, gameId: string): Promise<GameView> {
    ...
    const round = createRound(game.playerIds, this.#randomizer(game.playerIds.length), this.#shuffler)
    this.#liveRounds.set(game.id, round)
    game.round = toMemento(round)
    game.status = "PLAYING"
    await this.changed(game.id)
    return this.project(game, actor.id)
  }

  async playCard(token: string | undefined, gameId: string, cardIndex: number, selectedColor?: Color): Promise<GameView> {
    const actor = this.requirePlayer(token)
    ...
    await this.commitRound(game, play(cardIndex, selectedColor, round))
    return this.project(game, actor.id)
  }
  ...
  async drawCard(token: string | undefined, gameId: string): Promise<GameView> {
    const actor = this.requirePlayer(token)
    const { game, round } = this.roundForActor(gameId, actor.id, true)
    await this.commitRound(game, draw(round))
    return this.project(game, actor.id)
  }
  ...
  async sayUno(token: string | undefined, gameId: string): Promise<GameView> {
    ...
    await this.commitRound(game, sayUno(actorIndex, round))
    return this.project(game, actor.id)
  }
  ...
  async catchUno(token: string | undefined, gameId: string, accusedPlayerId: string): Promise<GameView> {
    ...
    await this.commitRound(game, catchUnoFailure({ accuser: actorIndex, accused: accusedIndex }, round))
    return this.project(game, actor.id)
  }
  ...
  subscribeGame(token: string | undefined, gameId: string): AsyncIterable<GameView> {
    ...
    return {
      async *[Symbol.asyncIterator]() {
        for await (const _ of source) yield service.project(service.gameById(gameId), viewer.id)
      },
    }
  }
  ...
  private roundForActor(gameId: string, actorId: string, requireTurn: boolean) {
    ...
    const round = this.#liveRounds.get(gameId) ?? createRoundFromMemento(game.round, this.#shuffler)
    this.#liveRounds.set(gameId, round)
  ...
  private project(game: StoredGame, viewerId: string): GameView {
    ...
    if (game.round === undefined) {
      return {
        ...base,
        players: game.playerIds.map(id => ({ id, username: this.playerById(id).username, cardCount: 0 })),
      }
    }

    const round = this.#liveRounds.get(game.id) ?? createRoundFromMemento(game.round, this.#shuffler)
    if (game.status === "PLAYING") this.#liveRounds.set(game.id, round)
    const discardTop = round.discardPile[0]
    return {
      ...base,
      players: game.playerIds.map((id, index) => {
        const cards = round.hands[index].map(copyCard)
        return id === viewerId
      ...
      playerInTurnId: round.playerInTurn === undefined ? undefined : game.playerIds[round.playerInTurn],
    }
  }
...
function normalizeUsername(username: string): string {
  ...
  if (normalized.length < 2 || normalized.length > 30) throw new Error("Username must contain between 2 and 30 characters")
  return normalized
}
