...
import { clearBrowserToken, writeBrowserToken } from "../shared/auth"
import {
  CATCH_UNO, CREATE_GAME, DRAW_CARD, GAMES, GET_GAME, JOIN_GAME, LOGIN, ME,
  PLAY_CARD, REGISTER, SAY_UNO, START_GAME,
} from "./graphql"
...
const initialState: AppState = { games: [], busy: false }
...
export const registerUser = createAsyncThunk<AuthPayload, { username: string; password: string }>("app/register", async variables => {
...
export const loginUser = createAsyncThunk<AuthPayload, { username: string; password: string }>("app/login", async variables => {
...
export const createGame = createAsyncThunk<GameView, { name: string; maxPlayers: number }>("app/createGame", async variables => {
...
export const playCard = createAsyncThunk<GameView, { gameId: string; cardIndex: number; color?: Color }>("app/playCard", async variables => {
...
export const catchUno = createAsyncThunk<GameView, { gameId: string; accusedPlayerId: string }>("app/catchUno", async variables => {
...
const asyncOperations = [restoreSession, registerUser, loginUser, loadGames, openGame, createGame, joinGame, startGame, playCard, drawCard, sayUno, catchUno] as const
...
const appSlice = createSlice({
  ...
  reducers: {
    ...
    leaveGameView(state) { state.currentGame = undefined; state.error = undefined },
    setClientError(state, action: PayloadAction<string>) { state.error = action.payload },
  ...
  extraReducers: builder => {
    builder
      ...
      .addMatcher(isPending(...asyncOperations), state => { state.busy = true; state.error = undefined })
      .addMatcher(isFulfilled(...asyncOperations), state => { state.busy = false })
      .addMatcher(isRejected(...asyncOperations), (state, action) => { state.busy = false; state.error = action.error.message ?? "Request failed" })
  },
})
...
export function makeStore(preloadedState: Partial<AppState> = {}) {
  const state: AppState = {
    ...initialState,
    ...preloadedState,
    games: preloadedState.games ?? initialState.games,
  }
  return configureStore({ reducer: appSlice.reducer, preloadedState: state })
}
