...
const noShuffle = (cards: ReadonlyArray<Card>): Card[] => cards.map(card => structuredClone(card))
const firstDealer = () => 0
...
describe("functional authoritative multiplayer service", () => {
  ...
  it("returns new functional round state instead of mutating prior projections", async () => {
    const { service, alice, bob } = await fixture()
    const game = await service.createGame(alice.token, "Immutable", 2)
    await service.joinGame(bob.token, game.id)
    const started = await service.startGame(alice.token, game.id)
    const current = started.playerInTurnId === alice.player.id ? alice : bob
    const before = structuredClone(started)
    await service.drawCard(current.token, game.id)
